...
# Build API server, static server (viewer on 8080), and CLI tools
RUN CGO_ENABLED=0 go build -ldflags="-s -w" -o api-server cmd/api-server/main.go && \
    CGO_ENABLED=0 go build -ldflags="-s -w" -o static-server cmd/static-server/main.go && \
...
COPY --from=builder /build/static-server /app/static-server
...
COPY verify_pass_tokens.sh update-pass-credential.sh docker-entrypoint.sh /app/
RUN chmod +x /app/verify_pass_tokens.sh /app/update-pass-credential.sh /app/docker-entrypoint.sh
